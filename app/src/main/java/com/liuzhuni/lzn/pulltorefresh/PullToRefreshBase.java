package com.liuzhuni.lzn.pulltorefresh;
...
import com.liuzhuni.lzn.pulltorefresh.ILoadingLayout.State;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    private volatile boolean showMore=false;
    ...
    @Override
    public final boolean onInterceptTouchEvent(MotionEvent event) {  //true 时消耗事件 false是 传递
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!onlyOne) {
                    if (isReadyForPullUp()&&absDiff > mTouchSlop&& deltaY<0)
                    {
                        showMore=true;
                        return true;
                    }
    ...
    public final boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                final float deltaUpY = ev.getY() - mLastMotionY;
                final float absDiffUp = Math.abs(deltaUpY);
                if (mIsHandledTouchEvent) {
                    mIsHandledTouchEvent = false;

//                    ToastUtil.show(getContext(), "what's wrong?  " + mIsHandledTouchEvent + "handle is " + handled);
                ...
                if (!onlyOne&&isReadyForPullUp()&&showMore) {
    ...
    public void setNoMore() {
        if (null != mFooterLayout) {
            mFooterLayout.setState(State.ADD_VIEW);
        }
    }
    ...
    public void addNewView(View view) {
        ...
        if (onlyOne) {
            ...
            if (null != mFooterLayout) {
                ...
                mViewHeight= DensityUtil.dip2px(getContext(),60);
            }
        }
    ...
    protected void resetNewViewLayout() {
//        if(mRefreshableViewWrapper.getBottom()>=mViewHeight){
//            onlyOne=true;
//        }else{
//            onlyOne=false;
//        }
        smoothScrollTo(mViewHeight, getSmoothScrollDuration(), 100);
    }
...
//                &&mRefreshableViewWrapper.getBottom()>=mViewHeight

//                Log.i("高度", "" + mCurrentY +"++++"+mViewHeight);

//                setScrollTo(0, mCurrentY);
                    if(!onlyOne&&showMore){
                        setScrollTo(0, mViewHeight);
                        stop();
                      }else{
                        setScrollTo(0, mCurrentY);
                    }
